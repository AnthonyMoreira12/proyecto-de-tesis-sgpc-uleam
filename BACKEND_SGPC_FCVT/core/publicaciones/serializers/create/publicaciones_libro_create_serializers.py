...
class LibroRegistroSerializer(PublicacionCamposBaseMixin, serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if "pais" in self.initial_data or "ciudad" in self.initial_data:
            raise ValidationError(
                {"detail": "País/Ciudad no aplican a Libros. Solo a Ponencias."}
            )
    ...
    def create(self, validated_data):
        ...
        libro = Libro.objects.create(
            ...
            editorial_compilador=validated_data["editorial_compilador"],
            revisor_par_arbitraje=validated_data["revisor_par_arbitraje"],
            link_libro=validated_data["link_libro"],
        )
