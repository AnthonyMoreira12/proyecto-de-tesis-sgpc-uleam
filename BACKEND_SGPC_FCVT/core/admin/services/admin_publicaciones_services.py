...
def _normalize_authors(
    ...
    """
    Normaliza la lista de autores sin jerarquías de autoría.

    La posición recibida se conserva como orden bibliográfico.
    El autor vinculado al usuario objetivo debe formar parte de la
    publicación; si el frontend no lo envía, se agrega al final para
    no alterar el orden de los autores que sí fueron especificados.
    """
    normalized = []
    seen_ids = set()
    ...
    if target_author.pk not in seen_ids:
        normalized.append(
            {
                "autor_id": target_author.pk,
                "orden": len(normalized) + 1,
            }
        )
